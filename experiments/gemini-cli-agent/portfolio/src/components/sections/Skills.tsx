...
// Skill data with connections and proficiency
interface Skill {
  id: string;
  name: string;
...
// Pre-calculate node positions in a circular/organic layout
...
function NeuralNode({
  ...
  const size = 28 + skill.proficiency * 6; // 34-58px based on proficiency
...
export function Skills() {
  ...
  // Calculate animation order based on x position (left to right)
  ...
  // Calculate reverse animation order (right to left for exit)
  const reverseAnimationOrder = useMemo(() => {
    const order: Record<string, number> = {};
    const sortedSkills = [...skills].sort((a, b) => {
      const posA = positions[a.id];
      const posB = positions[b.id];
      return posB.x - posA.x; // Reverse sort
    });
    sortedSkills.forEach((skill, index) => {
      order[skill.id] = index;
    });
  ...
  // Memoize pulse delays to keep them stable
